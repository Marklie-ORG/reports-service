...
export class CustomFormulasService {
  ...
  async createCustomFormula(formula: CreateAdAccountCustomFormulaRequest): Promise<AdAccountCustomFormula> {
    const adAccount = await database.em.findOne(ClientAdAccount, { adAccountId: formula.adAccountId });
  ...
  async getCustomFormula(uuid: string): Promise<AdAccountCustomFormula> {
    const customFormula = await database.em.findOne(AdAccountCustomFormula, { uuid });
    if (!customFormula) throw new Error(`Custom formula ${uuid} not found`);
    return customFormula;
  }

  async getAdAccountCustomFormulas(adAccountId: string): Promise<AdAccountCustomFormula[]> {
    return database.em.find(AdAccountCustomFormula, { adAccount: adAccountId });
  }

  async updateCustomFormula(uuid: string, formula: UpdateAdAccountCustomFormulaRequest): Promise<AdAccountCustomFormula> {
    const customFormula = await database.em.findOne(AdAccountCustomFormula, { uuid });
    if (!customFormula) throw new Error(`Custom formula ${uuid} not found`);
